...
if __name__ == "__main__":
    ...
    edited_funghi_type_dict = primary_data_gen. \
        get_funghi_type_dict_from_csv(data_cat.FILE_PATH_NA_PRIMARY, 0, 99)
    ...
    write_to_csv(data_cat.FILE_PATH_NA_SECONDARY_GENERATED, funghi_entry_list, False)

    # read secondary data as pandas.DataFrame, shuffle and write to a new CSV
    data_secondary = pd.read_csv(data_cat.FILE_PATH_NA_SECONDARY_GENERATED,
                                 sep=';', header=0, low_memory=False)
    data_secondary = data_secondary.sample(frac=1, random_state=1)
    data_secondary.to_csv(data_cat.FILE_PATH_NA_SECONDARY_SHUFFLED, sep=',', index=False)
